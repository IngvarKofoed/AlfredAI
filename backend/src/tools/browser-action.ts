...
const validateBrowserActionParameters = (parameters: Record<string, any>): { isValid: boolean; error?: string } => {
    ...
    const validActions = ['launch', 'scroll_down', 'scroll_up', 'close'];
...
export const browserActionTool: Tool = {
    description: {
        name: 'browserAction',
        description: 'Request to interact with a chrome extension controlled browser. Every action, except `close`, will be responded to with the html of the browser\'s current state. You may only perform one browser action per message, and wait for the user\'s response including a html to determine the next action.',
        parameters: [
            {
                name: 'action',
                description: 'The action to perform. Available actions: launch (must be first), scroll_down, scroll_up, close (must be last)',
                usage: 'Action to perform (e.g., launch, scroll_down, scroll_up, close)',
                required: true,
            },
            {
                name: 'url',
                description: 'The URL to launch the browser at. Required for launch action. Ensure the URL is valid and includes the appropriate protocol.',
                usage: 'URL to launch the browser at',
                required: false,
            },
            {
                name: 'pages',
        ...
        try {
            ...
            if (action === 'scroll_down') {
                ...
                try {
                    ...
                    return {
                        success: true,
                        result: `Scrolled down ${scrollPages} page(s). Current page HTML: ${htmlResponse}`
                    };
                } catch (error) {
                    return {
                        success: true,
            ...
            if (action === 'scroll_up') {
                ...
                try {
                    ...
                    return {
                        success: true,
                        result: `Scrolled up ${scrollPages} page(s). Current page HTML: ${htmlResponse}`
                    };
                } catch (error) {
                    return {
                        success: true,
